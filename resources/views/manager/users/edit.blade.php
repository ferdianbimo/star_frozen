...
<head>
    ...
    <title>Edit User - Star Frozen POS</title>
                    ...
                    <form method="POST" action="{{ route('manager.users.update', $user) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required
                                class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all hover:border-slate-300 @error('email') border-red-500 @enderror">
                            @error('email')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @error('password')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <p class="text-gray-500 text-xs mt-1">Minimum 8 characters if changing</p>
                        </div>
