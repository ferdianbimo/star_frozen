...
@section('content')
                <!-- Header -->
                <div class="bg-white rounded-xl p-4 lg:p-5 mb-4 lg:mb-6 border border-slate-200">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 lg:gap-4">
                        <div class="flex items-center gap-3 lg:gap-4">
                            <div class="w-10 h-10 lg:w-12 lg:h-12 bg-blue-500 rounded-xl flex items-center justify-center">
                                <i class="fas fa-cash-register text-lg lg:text-xl text-white"></i>
                            </div>
                            <div>
                                <h2 class="text-lg lg:text-xl font-bold text-slate-800">Point of Sale</h2>
                                <p class="text-slate-500 text-xs lg:text-sm">Transaksi penjualan cepat</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 w-full sm:w-auto">
                            <div class="flex-1 sm:flex-none px-3 lg:px-4 py-2 bg-slate-50 rounded-xl border border-slate-200">
                                <span id="realtimeClock" class="text-slate-600 text-xs lg:text-sm font-mono"><i class="fas fa-clock mr-1 lg:mr-2"></i>{{ now()->format('d M Y, H:i:s') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
                    ...
                    <div class="lg:col-span-8 bg-white rounded-2xl border border-slate-100 shadow-sm p-4 lg:p-5 overflow-auto order-2 lg:order-1 min-h-[300px] max-h-[calc(100vh-300px)] lg:max-h-[calc(100vh-200px)]">
                        ...
                        <div id="products" class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-4">
                            @foreach($products as $product)
                                <div class="bg-white border border-slate-200 rounded-xl p-3 lg:p-4 product-card hover:border-blue-300 hover:shadow transition-all duration-200" data-name="{{ strtolower($product->name) }}" data-product-id="{{ $product->id }}">
                                    <div class="flex flex-col items-center">
                                        <div class="w-20 h-20 lg:w-28 lg:h-28 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-center mb-2 lg:mb-3 overflow-hidden">
                                            @if($product->image)
                                        ...
                                        <div class="w-full mt-2 lg:mt-3">
                                            ...
                                            <button type="button" onclick="openBatchModal({{ $product->id }}, '{{ $product->name }}')" class="w-full inline-flex items-center justify-center gap-1 lg:gap-2 px-3 lg:px-4 py-2 lg:py-2.5 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-colors text-sm">
                    ...
                    <div class="lg:col-span-4 order-1 lg:order-2">
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 lg:p-5 mx-auto lg:sticky lg:top-4" style="max-height: 80vh;">
                            ...
                            <div class="mt-4 border-t border-slate-200 pt-4">
                                <form id="checkoutForm" method="POST" action="{{ route('cashier.pos.checkout') }}">
                                    ...
                                    <div class="mt-4">
                                        <button id="openCheckoutModal" type="button" class="w-full py-3 bg-blue-500 text-white rounded-xl font-semibold hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" {{ empty($cart) ? 'disabled' : '' }}>
...
<div id="checkoutModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-4 sm:p-6 my-auto">
        ...
        <div id="receiptDetails" class="text-sm text-slate-700">
            ...
            <div id="changeWrapper" class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex justify-between items-center">
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button id="checkoutConfirm" class="px-5 py-2.5 rounded-xl bg-emerald-500 text-white font-medium hover:bg-emerald-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2" disabled>
...
<div id="batchModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-4 sm:p-6 my-auto">
        ...
        <div class="flex justify-end gap-3">
            ...
            <button id="batchConfirm" class="px-5 py-2.5 rounded-xl bg-blue-500 text-white font-medium hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2" disabled>
